...
with st.sidebar:
    ...
    if not st.session_state.ai.rules_loaded:
        st.warning("⚠️ RAG Layer Offline: Check knowledge_base folder.")
    ...
    with st.expander("➕ Add Another Pet"):
        with st.form("add_pet_form", clear_on_submit=True):
            ...
            new_p_spec = st.selectbox("Species", ["Dog", "Cat", "Bird", "Other"])
...
with st.form("task"):
    ...
    title_in = st.text_input("What is the task? (e.g., Morning walk)").strip()
...
if not all_tasks:
    ...
    for idx, (pet, task) in enumerate(all_tasks):
        c1, c2, c3, c4 = st.columns([1, 3, 1, 1])
        c1.write(f"**{task.time}**")
        c2.write(f"{pet.name}: {task.title}")
        
        # Unique keys fix the crash and keep buttons visible
        if c3.button("Done", key=f"done_{task.time}_{idx}"):
            task.mark_complete()
            PetService.atomic_save(owner.to_dict(), DATA_FILE)
            st.rerun()
            
        if c4.button("🗑️", key=f"del_{task.time}_{idx}"):
